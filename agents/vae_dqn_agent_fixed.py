...
class VaeDqnAgent(ChefsHatPlayer):
    ...
    def get_reward(self, envInfo):
        """
        Graded terminal reward using OUR score (player index 0).
        score: 3=chef (+1.0), 2=sous_chef (+0.5), 1=waiter (-0.5), 0=dishwasher (-1.0)
        """
        scores = envInfo.get("Match_Score", [-1]*4)
        score  = scores[0] if len(scores) > 0 else -1   # always player 0
    ...
    def update_end_match(self, envInfo):
        ...
        # Compute our terminal reward using our own score
        scores  = envInfo.get("Match_Score", [-1]*4)
        score   = scores[MY_IDX] if MY_IDX < len(scores) else -1
        terminal_map = {3: 1.0, 2: 0.5, 1: -0.5, 0: -1.0}
        terminal_reward = terminal_map.get(int(score), -1.0)
        ...
        # Record finishing position correctly
        role_map = {3:"chef", 2:"sous_chef", 1:"waiter", 0:"dishwasher"}
        self.win_counts[role_map.get(int(score), "dishwasher")] += 1
