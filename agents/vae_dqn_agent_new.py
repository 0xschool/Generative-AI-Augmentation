...
class VaeDqnAgent(ChefsHatPlayer):
    ...
    def get_reward(self, envInfo):
        """Official reward — same as agent_random.py."""
        this_player  = envInfo["Author_Index"]
        position     = 3 - envInfo["Match_Score"][this_player]
        is_finished  = envInfo["Finished_Players"][this_player]
        return self.reward_fn.getReward(position, is_finished)
