...
class VaeDqnAgent(ChefsHatPlayer):
    ...
    def get_reward(self, envInfo):
        """
        Use Current_Roles NOT Match_Score.

        env.step() calls start_match_handle_cards() which resets current_score=-1
        BEFORE the info dict is built, so Match_Score is always [-1,-1,-1,-1].
        current_role is NOT reset by start_match_handle_cards, so Current_Roles
        is always correct at match end.

        ROLES = {0:"Dishwasher", 1:"Waiter", 2:"Souschef", 3:"Chef"}
        Current_Roles[0] is our agent's role (we are always player 0).
        """
        roles   = envInfo.get("Current_Roles", ["Dishwasher"]*4)
        my_role = roles[0] if roles else "Dishwasher"
        role_reward = {
            "Chef":      1.0,
            "Souschef":  0.5,
            "Waiter":   -0.5,
        ...
        return role_reward.get(my_role, -1.0)
    ...
    def update_end_match(self, envInfo):
        """
        Called at match end for ALL players with the same info dict.

        CRITICAL: Author_Index = whoever took the LAST action of the match,
        NOT our agent. Since our agent is always added as player 0, we
        hardcode MY_IDX = 0 to read our own score correctly.
        """
        MY_IDX = 0   # our agent is always player 0

        # Use Current_Roles — Match_Score is reset to -1 before info dict is built
        roles    = envInfo.get("Current_Roles", ["Dishwasher"]*4)
        my_role  = roles[MY_IDX] if MY_IDX < len(roles) else "Dishwasher"
        role_reward = {"Chef": 1.0, "Souschef": 0.5, "Waiter": -0.5, "Dishwasher": -1.0}
        role_wins   = {"Chef": "chef", "Souschef": "sous_chef", "Waiter": "waiter", "Dishwasher": "dishwasher"}

        terminal_reward = role_reward.get(my_role, -1.0)
        ...
        self.win_counts[role_wins.get(my_role, "dishwasher")] += 1
